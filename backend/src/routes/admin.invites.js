...
/**（可选）管理员鉴权：按你的项目替换 */
// function requireAdmin(req, res, next) {
//     // 如果你已有 JWT 中间件把用户放到 req.user，这里检查角色
//     if (process.env.ALLOW_OPEN_INVITE_GENERATE === 'true') return next(); // 本地调试开关
//     if (!req.user || req.user.role !== 'admin') {
//         return res.status(403).json({ error: '需要管理员权限' });
//     }
//     next();
 ...
 * body: { count=100, maxUses=1, expiresAt?, role='user', note? }
 * 返回：{ codes: string[] }  —— 明文只在这一次返回
 */
router.post('/admin/invites/generate', requireAdmin, async (req, res) => {
    try {
        const { count = 100, maxUses = 1, expiresAt, role = 'user', note } = req.body || {};

        const { codes } = await InviteCode.generateBatch({
            count: Number(count),
            maxUses: Number(maxUses),
            expiresAt: expiresAt ? new Date(expiresAt) : undefined,
            role,
            note,
            createdBy: req.user?._id, // 若无 req.user 可留空
        });
